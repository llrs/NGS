...
library("sva")
n.sv1 <- num.sv(v1$E, design, "leek")
n.sv2 <- num.sv(v2$E, design2, "leek")
if (n.sv1 >= 1) {
  sv1 <- sva(v1$E, design, n.sv = n.sv1)
  head(sv1$sv)
}
if (n.sv2 >= 1) {
  sv2 <- sva(v2$E, design2, n.sv = n.sv2)
  head(sv2$sv)
}
```
...
```{r p-values-sva}
if (n.sv2 >= 1) {
  pValues <- f.pvalue(v1$E, i.design, design)
  qValues <- p.adjust(pValues, method = "BH")
  hist(qValues)
}
if (n.sv2 >= 1) {
  pValues <- f.pvalue(v2$E, i.design2, design2)
  qValues <- p.adjust(pValues, method = "BH")
  hist(qValues)
}
```
...
We can group genes by function using [geneSCF](http://genescf.kandurilab.org/), which is a program written in perl. As input it requires a list of genes with each line the name of the gene:
