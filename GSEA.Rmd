...
gs_tiK <- GeneSet(unique(names(K.ok[increasing][1:100])), 
                 setName = "Top100_Increasing_K")
gs_tdK <- GeneSet(unique(names(K.ok[decreasing][1:100])), 
...
fgsea(pathways = geneIds(gsc_network), stats = stats, minSize = 15, 
                 maxSize = Inf, nperm = 10000)
...
tests <- sapply(names(gsc_late), function(x) {
  fisher.test(contingency(gsc_late[[x]], RD_signature_dw, size_DR))$p.value})
tests <- p.adjust(tests)
tests[tests < 0.05]

tests <- sapply(names(gsc_early), function(x) {
  fisher.test(contingency(gsc_early[[x]], RD_signature_dw, size_DR))$p.value})
tests <- p.adjust(tests)
tests[tests < 0.05]
...
pt.AHsevere <- read.csv("fgsea_ASH.csv")

extract_RD <- function(gsea) {
  gsea[c(grep("Positive_FC", gsea[, "pathway"]),
         grep("Negative_FC", gsea[, "pathway"]),
         grep("creasing_K", gsea[, "pathway"])), c(1, 3, 5)]
}
# Excract the data
...
# Plotting
ggplot(DR_progress) + geom_point(aes(x = Phase, y = NES, group = pathway, 
...
extract_RD <- function(gsea) {
  gsea[c(grep("Positive_FC", gsea[, "X"]),
         grep("Negative_FC", gsea[, "X"]),
         grep("creasing_K", gsea[, "X"])), c(1, 2, 6)]
}
# Extract the data
...
DR_progress <- rbind(fc.RD.ASH, fc.RD.Cirr, fc.RD.AH, fc.RD.AHsevere)
colnames(DR_progress) <- c("Gene_sets", "logFC", "adj.P.Val", "Phase")
# Plotting
ggplot(DR_progress) + geom_point(aes(x = Phase, y = logFC, group = Gene_sets, 
                                     color = Gene_sets, size = adj.P.Val)) + 
...
ggplot(DR_progress) + geom_point(aes(x = Phase, y = logFC, group = Gene_sets, 
                                     color = Gene_sets, size = adj.P.Val)) + 
  geom_line(aes(x = Phase, y = logFC, group = Gene_sets, color = Gene_sets)) +
  labs(title = "logFC of the gene set betweeen phases") + theme_bw()
```
