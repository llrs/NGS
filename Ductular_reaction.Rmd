...
plotMDS(DR, top = Inf, labels = c(seq(1, 6), seq(1, 6), seq(3, 6)), main = "Samples relationships")
```
...
colnames(logCPM) <- c(seq(1, 6), seq(1, 6), seq(3, 6))
d <- as.dist(1 - cor(logCPM, method = "spearman"))
cluster <- hclust(d)
plot(cluster)
...
design <- cbind(design, "Patient" = c(seq(from = 1, to = 6, by = 1),
...
```{r qualtiy, fig.width = 30, fig.height = 30 }
par(mfrow = c(3, 3))
...
showSigOfNodes(GOdata.bp, score(resultKS.weight), firstSigNodes = 10, 
               useInfo = 'all')
